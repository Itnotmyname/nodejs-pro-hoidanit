...
const initDatabase = async () => {
    const countUser = await prisma.user.count();// Đếm số lượng user, role, product trong database
    const countRole = await prisma.role.count();// Đếm số lượng user, role, product trong database
    const countProduct = await prisma.product.count();// Đếm số lượng user, role, product trong database
    ...
    } //Dòng if ở 52 đến 54 có thể bỏ vì ở phía dòng 164 đến 166 đã bao trọn cả trường hợp này rồi
